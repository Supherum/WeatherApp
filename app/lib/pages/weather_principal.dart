...
class WeatherPrincipal extends StatefulWidget {
  WeatherPrincipal({Key? key}) : super(key: key);
...
class _WeatherPrincipalState extends State<WeatherPrincipal> {
  ...
  void _initPreferences() async {
    final latitud = await PreferenceUtils.getString(LAT);
    final longitud = await PreferenceUtils.getString(LON);
    setState(() {
            ...
            slivers: [
              ...
              SliverList(
                ...
                InformationItems().dateAndLocationWidget(
                    context,
                    _whatDayIsToday(
                        DateTime.fromMillisecondsSinceEpoch(info.dt * 1000)),
                    info.name.toString()),
                Divider(
                    height: 1, thickness: 1, color: ColorStyles.colorLightGrey),
                Container(
                                ...
                                if (snapshot.hasData) {
                                  return Container(
                                    ...
                                    padding: const EdgeInsets.symmetric(
                                        vertical: 8.0),
                                    child: _dailyWeather(snapshot.data!.daily),
                                  );
                                }
